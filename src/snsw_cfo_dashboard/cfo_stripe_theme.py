"""Reusable Stripe-inspired visual layer for local CFO dashboards.

This deliberately leaves dashboard data/HTML generation alone and applies a shared
CSS + font layer at render time, so the same theme can be rolled across existing
generators without rewriting their data logic.
"""
...
# Keep generated dashboards local-only: do not inject Google Fonts or any
# other external asset requests. The CSS still prefers Source Sans/Source Code
# Pro if installed locally, then falls back to system fonts.
FONT_LINK = ""


def theme_link() -> str:
    return f'<link rel="stylesheet" href="{THEME_CSS}">'


def apply_stripe_theme(html: str) -> str:
    """Inject the shared dashboard CSS and remove external font links.

    Safe to call multiple times: it will not duplicate links/classes. Handles
    both plain `<body>` and `<body class=...>` generated by one-off dashboards.
    """
    # Earlier generated dashboards included Google Fonts. These local CFO pages
    # must not make external requests, so strip those links during regeneration.
    ...
    def repl(match: re.Match[str]) -> str:
        ...
        if class_match:
            ...
            if "stripe-cfo" not in classes:
                classes.insert(0, "stripe-cfo")
            # De-duplicate classes while preserving order; older generated dashboards
            # had `stripe-cfo stripe-cfo` after repeated theme passes.
            deduped = list(dict.fromkeys(classes))
            return tag[:class_match.start()] + f"class={quote}{' '.join(deduped)}{quote}" + tag[class_match.end():]
        return tag[:-1] + ' class="stripe-cfo">'
    html = re.sub(r"<body\b[^>]*>", repl, html, count=1, flags=re.I)
    html = re.sub(r"font-family:'Source Sans 3'", "font-family:system-ui", html)
    html = re.sub(r"font-family:'Source Code Pro'", "font-family:ui-monospace", html)
    return html


def ensure_theme_file(out_dir: Path) -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    css_path = out_dir / THEME_CSS
    css_path.write_text(STRIPE_CFO_CSS, encoding="utf-8")
    return css_path
